...
def move_vehicles(assignment, vehicles, g, vmr, travel, t, joined_stops, lion_node, sim=None):
    ...
    for trips, vid in assignment:
        ...
        cost, path = travel(t, v, trips, must_travel=True)
        if path is None:
        ...
        for i, e in enumerate(edges):
            ...
            if i > 0 and weight + total_time > slackt:
                max_i = i + 1
                next_node = g.vertex_properties['_graphml_vertex_id'][e.target()]
                events_ix, tr = handle_passengers(path, next_node, events_ix, v, sim)
                passengers_to_remove += tr

                break
            elif i == 0 and weight + total_time > slackt:
                max_i = 1
                total_time = weight
                events_ix, tr = handle_passengers(path, next_node, events_ix, v, sim)
                passengers_to_remove += tr
...
def handle_passengers(path, next_node, events_ix, vehicle, sim):
    passengers_to_remove = []
    again = True
    while again:
        again = False
        ...
        if next_event[0] == next_node:
            ...
            if next_event[1] == 'p':
                ...
                passengers_to_remove.append((path[events_ix][0], vehicle["id"]))
            else:
                vehicle["passengers"].remove(path[events_ix][0])
                sim.dropped_passengers.append((sim.t, path[events_ix][0], vehicle["id"]))
